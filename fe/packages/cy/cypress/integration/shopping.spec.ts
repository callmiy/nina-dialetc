...
import { getCreateShoppingData } from "../support/create-shopping-data";
...
context("Shopping", () => {
  ...
  describe("create shopping", () => {
    it("success", () => {
      const unwrappedData = getCreateShoppingData();

      cy.wrap(unwrappedData).then(() => {
        ...
        cy.get("#" + shoppingBrandInputId)
          ...
          .should("have.value", "")
          .within(() => {
            // There should be 1 fetched brands and one empty value
            cy.get("." + shoppingBrandOptionSelector).should("have.length", 2);
          });
        ...
        cy.get("#" + shoppingArticleInputId)
          ...
          .should("have.value", "")
          .within(() => {
            cy.get("." + shoppingArticleOptionSelector).should(
              "have.length",
              1
            );
          });
